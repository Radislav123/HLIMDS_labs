...
class Network:
    ...
    def __init__(self, neurons_number, shape = (5, 7)):
        # x = 5, y = 7
        # w = 5, h = 7
        self.shape = shape
        ...
        self.neurons = [Neuron(number) for number in range(neurons_number)]
        for neuron in self.neurons:
            other_neurons = copy.copy(self.neurons)
            other_neurons.remove(neuron)
            neuron.weights = dict(zip(other_neurons, [0 for _ in range(neurons_number)]))
    ...
    def write_weights(self, path, hex_format = False):
        with open(path, 'w') as file:
            for weight in self.get_weights(True):
                if hex_format:
                    file.write(int(weight).to_bytes(1, "big", signed = True).hex().upper())
                else:
                    file.write(str(int(weight).to_bytes(1, "big", signed = True)))
